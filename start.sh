...
# Create JSON Schema
var=$(curl -d @purchaseOrder.json -H 'X-Registry-ArtifactType: JSON'  -H 'X-Registry-Name: PurchaseOrder'  -H 'X-Registry-ArtifactId: PurchaseOrderJSON'  -H 'X-Registry-Version: 1'  -H 'X-Registry-Description: Artifact description' -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN"  -sL -w "%{http_code}\n" -X POST $APICURIO_ENDPOINT/apis/registry/v2/groups/sales/artifacts -o /dev/null)
if [ "$var" == "200" ]; then
...
# Create XML Schema
var=$(curl -d @purchaseOrder.xsd -H 'X-Registry-ArtifactType: XSD' -H 'X-Registry-ArtifactId: PurchaseOrderXSD' -H 'X-Registry-Name: PurchaseOrder' -H 'X-Registry-Version: 1' -H 'X-Registry-Description: XSD Artifact description' -H 'Accept: application/json'  -H "Content-Type: application/xml" -H "Authorization: Bearer $TOKEN"  -sL -w "%{http_code}\n" -X POST $APICURIO_ENDPOINT/apis/registry/v2/groups/sales/artifacts -o /dev/null)
if [ "$var" == "200" ]; then
